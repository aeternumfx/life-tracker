<template>
  <dialog
    ref="dialogRef"
    class="rounded-xl p-6 w-[500px] max-w-full shadow-xl bg-[--color-surface] text-[--color-textPrimary]"
  >
    <div class="space-y-4">
      ...
      <input
        v-model="form.title"
        type="text"
        class="w-full p-2 border rounded bg-[--color-input] text-[--color-inputText]"
      />
      ...
      <div class="flex justify-between items-center mt-6">
        <button
          @click="deleteEvent"
          class="px-3 py-1 rounded text-sm text-[--color-warning] border border-[--color-warning]"
        >
          🗑 Delete Event
        </button>
        ...
        <div class="flex gap-2">
          ...
          <button @click="save" class="px-4 py-1 rounded text-sm text-white bg-[--color-primary]">
            Save and Exit
          </button>
        </div>
      </div>
...
function deleteEvent() {
  ...
  if (confirmed) {
    emit('delete', { id: form.id, softDelete: true })
    close()
  }
}
