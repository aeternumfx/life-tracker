...
import { hydrateSettings, saveAndCloseSettings } from '@/utils/moduleSettingsHelpers'

defineProps()
const emit = defineEmits(['close', 'refresh'])

const moduleId = 'task-panel'
const settingsStore = useModuleSettingsStore()

const settings = reactive(hydrateSettings(settingsStore, moduleId, {
  limitEvents: true,
  maxEvents: 5
}))

async function saveAndClose() {
  await saveAndCloseSettings(settingsStore, moduleId, settings, emit)
}
</script>